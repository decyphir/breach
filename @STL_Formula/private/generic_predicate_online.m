...
% don't like that... AD: So let's get rid of it :) 
% traj.time = [-1e99 traj.time 1e99];          % NM: when fixing that, don't forgot to change
% traj.X = [traj.X(:,1) traj.X traj.X(:,end)]; % GetTrajSensi. AD:  Right, can you have a look at it ?  
...
for ii_var = 1:numel(SigList)
    ...
    for jj = 1:numel(matches)
        ...
        Pts(1:Sys.DimX,:) = X;
        Fval = GetF(Sys, time, Pts); % ask for derivatives à time instant "time"

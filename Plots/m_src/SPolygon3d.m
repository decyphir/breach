function [fv S] = SPolygon3d(Sys,S,timespan)
% -
% fv = SPolygon3d(Sys, S, timespan, option)
%
    ...
    switch (numel(S.dim))
     case 2
      for i = 0:size(S.pts,2)-1 % loop on trajectories
        ...
        for j = 1:lengthX
          vertices = reshape(X(:,j),3,[])';
      %   face = convhulln(vertices)+size(fv.vertices,1);
          face = [1 3 4 2 1]+size(fv.vertices,1);

...
/*
 * -------------------------------------
 * Added: Minimum stuff for quick Reinit
 * -------------------------------------
 */
...
int CVM_Initialization(int action, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  /* - Modif -  Now in global structure (except Vabstol)
  ...
  */
  ...
  case 0:
    ...
    /* - Modif: if empty is provided as RHS function, init with f in dynamics.h */
    /*          else use matlab function                                        */

      // status = CVodeMalloc(cvode_mem, mtlb_CVodeRhs, t0, y, itol, reltol, abstol);
    
      if (mxIsEmpty(mx_RHSfct)) {
	status = CVodeMalloc(cvode_mem, f, t0, y, itol, reltol, abstol);
	InitFdata(f_data, mx_data);
  ...
  case 1:
    ...
    if (mxIsEmpty(mx_RHSfct)) {
      status = CVodeReInit(cvode_mem, f, t0, y, itol, reltol, abstol);
  ...
  /* - Modif: still need tolerances for quick reinit  */
...
int CVM_SensInitialization(int action, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  /* - Modif:  yS0 now a N_Vector* in global structure */
  ...
  bufval = (char*) mxCalloc(buflen, sizeof(char));
  ...
  if (action == 0) {
    ...
    for (is=0;is<Ns;is++) {
      PutData(yS[is], &yS0data[is*N], N);
      PutData(yS0[is], &yS0data[is*N], N);
    }
  }
  else
    for (is=0;is<Ns;is++) {
...
int CVM_SensToggleOff(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  int status;
...
int CVM_AdjInitialization(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  int buflen, status;
  char *bufval;
...
int CVM_InitializationB(int action, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  double tB0, *yB0;
  int status;
...
int CVM_Free(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  /* - Added - Destroy stored tolerances and f_data  */

  if (mxIsEmpty(mx_RHSfct))
    FreeFdata(f_data,mx_data);
  
  if  (itol==CV_SV)
    N_VDestroy(NV_abstol);
  
  if (cvm_fsa)
    N_VDestroyVectorArray(yS0, Ns);

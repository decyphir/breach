...
% Builds a set of nb^n points in N^n (nb points per axes)
%
%  l = VanDerCorp(nb,3);
...
function Nn= N2NnIter(n,p,nb)
  ...
  if (n==1)
    ...
    Nn = [ repmat(Nnm,1,nb(end));  
           kron(1:nb(end),ones(1,nbm))];
  end
...
function l = VanDerCorp(nb, base)
  ...
  l=[];
  for i=0:nb-1
    l = [l baseget(i,nbit,base)*inv_base']; %this is van der corput seq.
  end
